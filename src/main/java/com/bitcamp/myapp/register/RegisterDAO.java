...
public class RegisterDAO extends DBConnection implements RegisterDAOImpl {
	...
	public int insertRecord(RegisterVO vo) {
		...
		try {
			dbConn();
			sql = "insert into register(userid, userpwd, username, tel, "
					+ "email, zipcode, addr, addrdetail, hobby) values(?,?,?,?,?,?,?,?,?)";
	...
	public int idDoubleCheck(String userid) {
		...
		try {
			...
			if(rs.next()) { //무조건 0아니면 1

				cnt = rs.getInt(1); //첫번쨰 꺼 넘김 
